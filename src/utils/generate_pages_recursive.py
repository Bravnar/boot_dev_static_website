...
def recursive_generate(src, template, dest, basepath):
    ...
    for item in list_to_generate:
        ...
        if os.path.isfile(abs_path):
            ...
            generate_page(abs_path, template, dest_path, basepath)
        elif os.path.isdir(abs_path):
            new_dir_path = os.path.join(dest, item)
            os.mkdir(new_dir_path)
            recursive_generate(abs_path, template, new_dir_path, basepath)
...
def generate_pages_recursive(src_dir, template_path, dest_dir, basepath):
    ...
    recursive_generate(src_abs, template_abs, dest_abs, basepath)
